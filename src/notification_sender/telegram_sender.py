...
3. 可选: 自动置顶发送的消息 (TELEGRAM_PIN_REPORTS=true)
"""
import logging
import os
...
class TelegramSender:
    ...
    @staticmethod
    def _is_pin_enabled() -> bool:
        """Check if TELEGRAM_PIN_REPORTS env flag is set."""
        return os.environ.get('TELEGRAM_PIN_REPORTS', '').lower() in ('true', '1', 'yes', 'on')

    def _pin_telegram_message(self, bot_token: str, chat_id: str, message_id: int) -> bool:
        """Pin a message in Telegram chat via pinChatMessage API."""
        try:
            api_url = f"https://api.telegram.org/bot{bot_token}/pinChatMessage"
            payload = {
                "chat_id": chat_id,
                "message_id": message_id,
                "disable_notification": True,  # silent pin (no extra notification)
            }
            ...
            logger.warning(f"Telegram 置顶异常: {e}")
            return False
        ...
        for attempt in range(1, max_retries + 1):
            ...
            if response.status_code == 200:
                ...
                if result.get('ok'):
                    ...
                    # Auto-pin if enabled
                    if self._is_pin_enabled():
                        message_id = result.get('result', {}).get('message_id')
                        if message_id:
                            bot_token = api_url.split('/bot')[1].split('/')[0]
                            self._pin_telegram_message(bot_token, chat_id, message_id)
                    return True
                else:
        ...
        if response.status_code == 200:
            ...
            if result.get('ok'):
                ...
                if self._is_pin_enabled():
                    ...
                    if message_id:
                        bot_token = api_url.split('/bot')[1].split('/')[0]
                        chat_id = plain_payload.get('chat_id')
                        if chat_id:
                            self._pin_telegram_message(bot_token, str(chat_id), message_id)
                return True
